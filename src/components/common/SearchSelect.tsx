...
export const SearchSelectCustom = forwardRef<HTMLDivElement, SearchSelectCustomProps>(
  function SearchSelectCustom(
    ...
    /* Focus search + scroll dropdown into view on open */
    useEffect(() => {
      if (!open) return;
      setActiveIndex(0);
      const t = setTimeout(() => {
    ...
    const triggerBorder = error
      ...
      : open
        ? "border-primary shadow-[0_0_0_3px_rgba(113,42,226,0.10)]"
    ...
    return (
      <div
        ref={mergeRef}
        data-cy={dataCy}
        className={["flex flex-col gap-[0.8rem]", className].filter(Boolean).join(" ")}
      >
        {label && (
          <label
            htmlFor={id}
        ...
        <button
          ...
          className={[
            ...
            "rounded-[0.8rem] bg-background border",
        ...
        >
          ...
          <span className="flex items-center shrink-0 text-text-muted transition-transform duration-200"
        ...
        {/* Inline dropdown — no portal, no z-index fights */}
        {open && (
          <div
            ref={dropdownRef}
            role="listbox"
          ...
          >
            ...
            {/* Options */}
            <ul className="max-h-[20rem] overflow-y-auto py-[0.4rem]">
              {filtered.length === 0 ? (
